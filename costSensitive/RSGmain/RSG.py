...
class RSG(nn.Module):
    def __init__(self, n_center = 3, feature_maps_shape = [32, 16, 16], num_classes=10, contrastive_module_dim = 128, head_class_lists = [], transfer_strength = 1.0, epoch_thresh = 100):
        ...
        self.C, self.H, self.W = feature_maps_shape  # 获得特征图形状

        self.n_center = n_center  # 手动设定样本分布中心数目
        self.pooling = nn.AvgPool2d(self.H)  # 平均池化
 
        self.linear = nn.Parameter(torch.randn(num_classes, self.C, n_center).cuda()) # 根据类别数，通道数，中心书生成随机的全连接层线性参数，放在CPU里
        self.bias = nn.Parameter(torch.ones(num_classes, n_center).cuda())  # 生成偏置，全1矩阵

        self.centers = nn.Parameter(torch.zeros(num_classes, n_center, self.C).cuda())  # 初始化分布中心矩阵，全0
        self.softmax = nn.Softmax(dim=1)   # softmax层 维度1上相加为1
        self.strength = transfer_strength  # 转移强度参数
        self.epoch_thresh = epoch_thresh   # 生成样本阈值

        self.contrastive_module_dim = contrastive_module_dim  # 比较模块维度
        ...
        self.contrastive_fc = nn.Linear(self.contrastive_module_dim, 2)  # 全连接层 输入张量为定义的contrastive_module_dim， 输出为2
    ...
    def compute_cesc_loss(self, feature_maps, c, gamma, target, epoch):
        num, C, H, W = feature_maps.size()  # 获得输入特征图像的参数
        gamma = gamma.unsqueeze(1)  # 在gamma参数的1位置加一个维度
   
        if epoch <= self.epoch_thresh:
         if (num % 2 != 0):
             c1 = torch.zeros(1,C,H,W)
             feature1 = feature_maps[0:num // 2, :, :, :]  # 特征图数目的前一半
             feature1 = torch.cat([feature1,c1],dim=0)
             target1 = target[0: num // 2]  # 标签的前一半
             c2 = torch.zeros(1)
             target1 = torch.cat([target1,c2])
         else:
            feature1 = feature_maps[0:num//2, :, :, :]  # 特征图数目的前一半
            target1 = target[0: num // 2]  # 标签的前一半
         feature2 = feature_maps[num//2:num, :, :, :]  # 特征图数目的后一半


         #target1 = target[0 : num//2]  # 标签的前一半
         target2 = target[num//2 : num]  # 标签的后一半

         feature_cat = torch.cat([feature1, feature2], dim=1)  # 特征图按列进行拼接
         '''print(feature1)
         print(feature2)
         print(feature_cat)'''
         target_cat = torch.eq(target1, target2).long()  # 比较前半部分的标签和后半部分的标签，并将true/false转化为整数
        
         pair_fea = self.contrastive_module(feature_cat).contiguous().view(-1, self.contrastive_module_dim)  # 调用比较模块，并自动计算输出向量的size
         pair_pred = self.contrastive_fc(pair_fea)  # 计算预测输出
    ...
    def to_one_hot_vector(self, num_class, label):
        ...
        b = np.zeros((label.shape[0], num_class))  # 生成num_class*label列数的全0矩阵
        b[np.arange(label.shape[0]), label] = 1  # 相应标签位置置1
        b = torch.from_numpy(b)
        return b

    def compute_mv_loss(self, origin_feature, origin_center, target_center, target_features, gamma_head, target, gamma_tail):
        c = origin_center.detach()  # 原来分布中心的tensor c改变可以同时改变origin_center
        num, C, H, W = target_features.size()  # 获得标签向量的size
        gamma_h = gamma_head.detach()  # 获得gamma_head
        gamma_t = gamma_tail.detach()  # 获得gamma_tail
        ori_f = origin_feature.detach()  # 获得原特征图
        c_ = target_center.detach()  # 获得标签中心
        ...
        index = gamma_h.argmax(dim=1)  # gamma_h 维度1上找最大值位置
        index_ = gamma_t.argmax(dim=1)  # gamma_t 维度1上找最大值位置
        index = self.to_one_hot_vector(self.n_center, index).unsqueeze(1).cuda()  # 生成最大值的one_hot向量
        index_ = self.to_one_hot_vector(self.n_center, index_).unsqueeze(1).cuda()  # 同上

        c_o = torch.bmm(index, c.view(-1, self.n_center, self.H*self.W*self.C).double()).view(ori_f.size()).cuda()  # 计算参数
    ...
    def forward(self, feature_maps, head_class_lists, target, epoch):
        ...
        maps_detach = feature_maps.detach()  # 获得输入特征图

        total = target.size()[0]  # 获得标签的第一个维度
        ...
        #maps_detach_p = maps_detach.view(-1,self.C)
